"""Phase A-4: 테마별 실적 축 계산.

SPEC: docs/radar/SPEC_phase_a_signals.md §3
Phase A는 미국 시장만 대상. 분기 재무제표는 scripts/backfill_quarterly_financials.py로
미리 채워져 있어야 한다(승인된 테마 매핑 종목만 대상으로 함).
...
Usage:
  python scripts/compute_theme_earnings.py                       # 전체 US 테마
...
def main() -> None:
    ...
    # 테마별 소속 기업을 먼저 다 모아서, 필요한 티커 전체에 대해 재무 캐시를
    # 한 번만 벌크 조회한다 - 테마마다 개별 조회하면 겹치는 종목(여러 테마
    # 소속)을 중복으로 왕복하게 된다(SPEC §7과 동일한 "배치로" 원칙).
    ...
    for theme in themes:
        ...
        _log(f"{theme_id}: members={result['members']} improved={result['improved']} "
