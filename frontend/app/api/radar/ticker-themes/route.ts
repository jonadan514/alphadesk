...
// 조회 - radar/members가 테마→종목이라면 이건 종목→테마. Phase A는 미국 시장만.
    ...
    const res = await client.execute({
      sql: `
        WITH latest_runs AS (
          ...
          FROM theme_members
          WHERE market = 'US' AND approved = 1
          GROUP BY theme_id
        )
        SELECT tm.theme_id, ts.label
        FROM theme_members tm
        JOIN latest_runs lr ON lr.theme_id = tm.theme_id AND lr.run_id = tm.run_id
        LEFT JOIN theme_signals ts ON ts.theme_id = tm.theme_id AND ts.market = 'US'
          AND ts.week_start = (SELECT MAX(week_start) FROM theme_signals WHERE market = 'US')
        WHERE tm.ticker = ? AND tm.market = 'US' AND tm.approved = 1
