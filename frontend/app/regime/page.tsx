...
const KR_SENSOR_META: Record<string, { label: string; weight: string; desc: string }> = {
  trend:      { label: "TREND",      weight: "35%", desc: "KOSPI가 SMA200 위에 있는지. 장기 추세 방향" },
  volatility: { label: "VOLATILITY", weight: "25%", desc: "60일 변동성. 높을수록 시장 불안정 · 리스크 확대" },
  momentum:   { label: "MOMENTUM",   weight: "25%", desc: "20일 수익률. 단기 방향성과 모멘텀 강도" },
...
export default function RegimePage() {
  ...
  return (
    <div className="space-y-2">
      ...
      {/* US-only: macro snapshot */}
      {market === "US" && data.macro_snapshot && (
        <div className="bg-card rounded-xl p-3">
          <h2 className="stat-label mb-2">매크로 스냅샷</h2>
          <div className="grid grid-cols-5 gap-2">
            {Object.entries(data.macro_snapshot).map(([k, v]) => {
              ...
              return (
                <div key={k} className="rounded-lg p-2.5" style={{ background: "var(--bg-inset)", border: "1px solid var(--border)" }}>
                  <p className="text-[12px] font-semibold" style={{ color: "var(--text-muted)" }}>{meta.label}</p>
                  <p className="text-[18px] font-black text-white leading-none mt-1">{v != null ? String(v) : "—"}</p>
                  {meta.desc && (
                    <p className="text-[11px] mt-1.5 leading-snug" style={{ color: "var(--text-faint)" }}>{meta.desc}</p>
                  )}
                </div>
              );
            })}
