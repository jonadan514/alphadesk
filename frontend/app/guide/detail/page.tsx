      ...
      <>
        ...
        <Table
          ...
          rows={[
            ...
            ["실적", "소속 기업들의 최근 분기 매출 YoY 개선 비율", "과반 개선(↑)·그렇지 않음(↓)·표본부족(–)"],
        ...
        <Table
          headers={["라벨", "뜻"]}
          rows={[
            ["조용히 좋아짐", "뉴스는 잠잠하거나 줄었는데 실적은 실제로 개선 — 시장이 아직 못 알아챈 후보"],
            ["바닥 통과 가능", "뉴스도 주가도 안 좋은데 실적은 개선 — 시장이 아직 못 따라잡았을 가능성"],
            ["새로 부상", "뉴스가 급증했지만 실적은 아직 안 나옴"],
            ["관심 강화", "뉴스·실적·주가가 다 같은 방향으로 움직임"],
            ["과열 경계", "뉴스·주가는 뜨거운데 실적은 식음"],
            ["약화", "세 축 다 하락"],
          ]}
        />
        <Note>
        ...
        <P>
          ...
          여부를 그대로 가져온 것이고, &quot;미확인&quot;은 <Danger>탈락이 아니라</Danger> 이번 주
          갱신 대상이 아니었거나 유니버스 밖이라는 뜻입니다 — 판정은 항상 통과/탈락/데이터부족
          3분류이지, 확인 안 된 걸 탈락으로 단정하지 않습니다.
        </P>
        <Note>
          💡 아직 미국 시장만 지원합니다. 홈 화면과 사이드바 워치리스트 팝업(관련 테마 배지)에서도
          이번 주 라벨을 확인할 수 있습니다.
        </Note>
      </>
    ),
    ...
    content: (
      <>
        {[
          ...
          {
            q: "관심도 상승 배지나 테마 레이더의 '새로 부상' 라벨을 보고 바로 추격 매수한다",
