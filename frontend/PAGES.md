...
### 매수 체크 `/workflow`
주문 직전 최종 관문. 6단계 시장 신호 + 티커 입력 시 13개 조건 중 11개 자동 판정(게이트·체제·워치리스트 포함·BUY·등급·점수·선행섹터·AI·리스크·손절가·수량), 수동 2개(정성 검증 완료·여유 자금)는 클릭 체크. 내 워치리스트 종목 빠른 선택 칩. 포지션 사이징(고정 비율 역산 + 켈리 검증).
